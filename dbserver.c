...
int client_fd = *((int *) arg); //each thread has its own client_fd
...
ssize_t readSize = read(client_fd, &message, sizeof(message)); //we read the clients message, return how much we read
if (readSize == 0) {
    printf("[The client disconnected.] \n"); /**if we've read 0 bytes, we have reached EOF,
     meaning no data has been read, no message has been sent, the client must have disconnected, no further need to wait for a message from this client **/
    break;
}
if (readSize == -1) { //error checking, here we dont break just continue to next round, part of original code
...
if (readSize < sizeof(message)) { //make sure the server has read and recieved the entire message if not this error means incomplete message recieved!!
            response = FAIL; //signal the failure as it happened on the servers end (unlike previous error with input)
            write(client_fd, &response, sizeof(response)); //here, we are writing the response back to the client to tell them know what has happened
            continue; 
}
 ...
 switch (message.type) {
       case PUT:
       {
         ...
         if (db_fd == -1) {
           perror("failure to open database file for PUT message");
           response = FAIL; //again... failure on SERVER send so we send a response back
                    ...
                    perror("failure to store the record for PUT message");
                    response = FAIL;
                    }
             close(db_fd); //remember to close the fd when we don't need it
                }
       write(client_fd, &response, sizeof(response)); //send the response back to the client
      }
       break;
       
       case GET:
            {
             struct record tester; //we have to go through the database of records sequentially, this "tester" will have new data as we move across database
              ...
              response = FAIL; //initally set the response to fail 
                 int db_fd = open("database.dat", O_RDONLY); //keep opening and closing the file, DONT make it global!!
                    if (db_fd == -1) {
                        perror("failure to open database file for GET message");
                        //response is already set to FAIL, so we just don't change it at this point and skip over else block
                        }
                        else {
             
              while (read(db_fd, &tester, sizeof(struct record)) == sizeof(struct record)) { 
             //think of the tester as a record sliding through the database, we stop reading once we go to EOF, and we've read 0.
             //this works because the database is a sequential storage of records!
               if (tester.id == message.rd.id) { //for each record, check its message and see if it messages from the messages.rd.id
    ...
    write(client_fd, &response, sizeof(response)); //give response back to client, we only change it if we found a success, otherwise its false!
    /**it can be false because open failed, or because we just didnt find a record.
     for the first case however we've set a perror statement bc its a fault of a system call **/
     if (found) {
      write(client_fd, &tester, sizeof(tester)); //IF found is not 0, only then can we send the record as well (which is what tester will contain)
       }
    close(db_fd); 
    }
    break;
    ...
    default:
       {         response = FAIL;
                write(client_fd, &response, sizeof(response)); //just in case, default case return FAIL, if somehow the message type does not match
